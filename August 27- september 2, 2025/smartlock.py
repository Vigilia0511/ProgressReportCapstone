...
def enroll_voice_resemblyzer():
    ...
    if matched_id:
        ...
        speak(f"Voice already registered. Profile for user {matched_id} updated.")
        logger.info(f"Voice profile for user {matched_id} updated (Resemblyzer).")
        ...
        for i in range(1, 11):
            if i not in profiles:
                ...
                logger.info(f"Voice profile for user {i} enrolled and saved (Resemblyzer).")
            ...
            logger.warning("Voice enrollment failed: no available slots.")
            return False
...
def listen_for_command_resemblyzer():
    ...
    fs = 16000
    duration = 3
    speak("Speak now for verification.")
    audio = sd.rec(int(duration * fs), samplerate=fs, channels=1, dtype='float32')
    sd.wait()
    wav = np.squeeze(audio)
    wav = preprocess_wav(wav, source_sr=fs)
...
def main_loop():
    """Main program loop with improved fallback authentication logic and PIN restriction."""
    total_failures = 0
    max_failures = 3
    ...
    while True:
        try:
            ...
            # Reset attempted_methods and fallback_sequence if all are done or after unlock
            if len(attempted_methods) >= 3 or auth_state.unlock_complete:
                attempted_methods.clear()
                fallback_sequence = []
                auth_state.unlock_complete = False

            # Fingerprint first (automatic detection)
            try:
                if f.readImage() and not fallback_sequence:
                    logger.info("Fingerprint detected, attempting verification")
            ...
            if key:
                ...
                # Map keypad input to authentication method and fallback order
                if key == "1" and not fallback_sequence:
                    fallback_sequence = ["voice", "fingerprint", "face"]
                    attempted_methods.clear()
                elif key == "2" and not fallback_sequence:
                    fallback_sequence = ["face", "fingerprint", "voice"]
                    attempted_methods.clear()
                elif key == "A":
                    # Only allow PIN if all three have been attempted
                    if len(attempted_methods) >= 3:
                        ...
                        notification_manager.log_notification(user_id, "someone is trying to open the door with pin")
                        sleep(2)
                    continue
                else:
                    # Ignore other keys until a sequence is started
                    pass

                # Run fallback sequence if set and not finished
                if fallback_sequence and len(attempted_methods) < 3:
                    for method in fallback_sequence:
                        if method not in attempted_methods:
